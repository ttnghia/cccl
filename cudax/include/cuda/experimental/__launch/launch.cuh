...
{
  return __detail::driver::kernelGetFunction(__kernel.get());
}
...
__do_launch(_GraphInserter&& __inserter, CUlaunchConfig& __config, CUfunction __kernel, void** __args_ptrs)
{
  ...
  const auto __node = __detail::driver::graphAddKernelNode(
  ...
  {
    __detail::driver::graphKernelNodeSetAttribute(__node, __config.attrs[__i].id, __config.attrs[__i].value);
  }
  ...
  cuda::stream_ref __stream, CUlaunchConfig& __config, CUfunction __kernel, void** __args_ptrs)
{
  ...
  __detail::driver::launchKernel(__config, __kernel, __args_ptrs);
...
_CCCL_HOST_API auto __launch_impl(_Dst&& __dst, _Config __conf, CUfunction __kernel, _ExpTypes... __args)
{
  ...
  return __do_launch(_CUDA_VSTD::forward<_Dst>(__dst), __config, __kernel, const_cast<void**>(__pArgs));
}
